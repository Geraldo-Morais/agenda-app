document.addEventListener('DOMContentLoaded', () => {
    ...
    let ultimaConcluidasIds = [];
    let concluidasDocRef = null;
    ...
    async function salvarConcluidas() {
        const ids = Array.from(document.querySelectorAll('.atividade.concluida')).map(el => el.dataset.id);
        try {
            await db.collection('concluidas').doc(getChaveDeHoje()).set({ ids });
            ultimaConcluidasIds = ids;
        } catch (error) {
    ...
    function destacarAtividadeAtual() {
        ...
        const nomeDiaHoje = mapaDosDias[agora.getDay()];
    ...
    function renderizarAgenda() {
        ...
        if (modoEdicao) {
            document.querySelectorAll('.btn-remover').forEach(btn => btn.addEventListener('click', removerAtividade));
        }
        if (ultimaConcluidasIds && ultimaConcluidasIds.length) {
            setTimeout(() => aplicarConcluidas(ultimaConcluidasIds), 0);
        }
    ...
    function copiarAtividades(evento) {
        ...
        atividadesCopiadas.forEach((atividade, index) => { atividade.id = `${diaDestino}-${Date.now()}-${index}`; });
        agenda[diaDestino] = atividadesCopiadas;
        fecharModalCopia();
        renderizarAgenda();
    }

    function adicionarAtividadeSimples(nomeDia) {
        const novaAtividade = { id: `${nomeDia}-${Date.now()}`, descricao: 'Nova Atividade', inicio: '', fim: '' };
        if (!agenda[nomeDia]) agenda[nomeDia] = [];
        if (agenda[nomeDia].length === 1 && agenda[nomeDia][0].descricao === 'Dia de descanso') { agenda[nomeDia] = []; }
        agenda[nomeDia].push(novaAtividade);
        renderizarAgenda();
    }

    function removerAtividade(evento) {
        const idParaRemover = evento.target.dataset.id;
        const nomeDia = evento.target.dataset.dia;
        agenda[nomeDia] = agenda[nomeDia].filter(atividade => atividade.id !== idParaRemover);
        renderizarAgenda();
    }
    ...
    function atualizarConcluidasListener() {
        if (concluidasListener) { concluidasListener(); concluidasListener = null; }
        concluidasDocRef = db.collection('concluidas').doc(getChaveDeHoje());
        concluidasListener = concluidasDocRef.onSnapshot((concluidasDoc) => {
            const ids = concluidasDoc.exists ? (concluidasDoc.data().ids || []) : [];
            ultimaConcluidasIds = ids;
            setTimeout(() => aplicarConcluidas(ids), 0);
        }, (error) => {
            console.error("Erro ao ouvir concluídas: ", error);
        });
    }
